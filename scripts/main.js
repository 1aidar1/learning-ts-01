    ...
    class NeuralNetwork {
        init(neuronCounts) {
            ...
            for (let i = 0; i < neuronCounts.length - 1; i++) {
                levels.push(new Level(neuronCounts[i], neuronCounts[i + 1]));
            }
...
define("car", ["require", "exports", "controller", "sensor", "geometry", "network"], function (require, exports, controller_1, sensor_1, geometry_3, network_1) {
    ...
    class Car {
        constructor(x, y, width, height, controllerType, maxSpeed = 3, acceleration = 0.2) {
            ...
            switch (controllerType) {
                ...
                case controller_1.ControlType.AI:
                    this.sensor = new sensor_1.Sensor();
                    const net = new network_1.NeuralNetwork().init([this.sensor.rayCount, 6, 4]);
                    this.controller = new controller_1.AIController(net);
                    break;
                default:
        ...
        setController(controller) {
            console.log(controller instanceof controller_1.AIController);
...
define("main", ["require", "exports", "road", "car", "controller", "network"], function (require, exports, road_1, car_1, controller_2, network_2) {
    ...
    if (storedItem != null) {
        const brain = JSON.parse(storedItem);
        bestCar.controller = new controller_2.AIController(brain);
    }
    const traffic = [
    ...
    function generateCars(n) {
        const cars = [];
        for (let i = 0; i < n; i++) {
            cars.push(new car_1.Car(road.getLaneCenter(Math.floor(road.laneCount / 2)), 200, 30, 50, controller_2.ControlType.AI));
        }
        return cars;
    }
    function animate() {
        if (mainCtx) {
            mainCanvas.height = window.innerHeight;
            mainCtx.save();
            bestCar = cars.find(c => c.coordinates.y == Math.min(...cars.map(c => c.coordinates.y)));
